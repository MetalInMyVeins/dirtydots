...
-- HexEdit take filename as input
-- The file being input contains the path to be passed to HexEdit
vim.keymap.set("n", "<leader>he", function()
    vim.ui.input({ prompt = "Path file: " }, function(filename)
        if not filename or filename == "" then
            return
        end

        local path = vim.fn.readfile(filename)[1]

        if not path or path == "" then
            vim.notify("File is empty or unreadable", vim.log.levels.ERROR)
            return
        end

        vim.cmd("HexEdit " .. vim.fn.fnameescape(vim.fn.expand(path)))
    end)
end)
...
-- vim.opt.listchars = { tab = "▸ " }   -- solid arrow
-- vim.opt.listchars = { tab = "→ " }   -- thin arrow
-- vim.opt.listchars = { tab = "» " }   -- double angle
-- vim.opt.listchars = { tab = "⇥ " }   -- tab symbol
...
vim.api.nvim_set_hl(0, "@attribute.cpp", { fg="Purple3", bold=true })
...
vim.api.nvim_set_hl(0, "@number.c", { fg="Magenta3", bold=true })
